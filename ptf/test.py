...
class PtfTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        forward_table.entry_add(
            target,
            [forward_table.make_key([gc.KeyTuple("hdr.ethernet.dst_addr", dmac_m1)])],
            [forward_table.make_data(
                [gc.DataTuple('eg_port',eg_port_m1)],
                'SwitchIngress.set_eg_port')])
        
        forward_table.entry_add(
            target,
            [forward_table.make_key([gc.KeyTuple("hdr.ethernet.dst_addr", dmac_h4)])],
            [forward_table.make_data(
                [gc.DataTuple('eg_port',eg_port_h4)],
                'SwitchIngress.set_eg_port')])
